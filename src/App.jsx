...
const ThemeProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (theme === 'dark') {
      ...
      // LIGHT MODE (PORCELAIN & CRYSTAL)
      root.style.setProperty('--bg-color', '#F5F5F7'); // Off-white Premium (Apple style)
      root.style.setProperty('--text-color', '#1D1D1F'); // Quase preto, mas suave
      root.style.setProperty('--glass-bg', 'rgba(255, 255, 255, 0.75)'); // Mais sólido para contraste
      root.style.setProperty('--glass-border', 'rgba(0, 0, 0, 0.05)'); // Borda escura sutil (essencial no light)
      root.style.setProperty('--glass-shadow', 'rgba(0, 0, 0, 0.05)'); // Sombra suave difusa
      root.style.setProperty('--accent-color', '#0071E3'); // Azul vibrante
      root.style.setProperty('--muted-text', '#86868b');
    }
  }, [theme]);
    ...
    <a href={href} target="_blank" rel="noreferrer" style={{ textDecoration: 'none', cursor: 'none', width: '100%' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', zIndex: 1 }}>
          <span style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text-color)' }}>{label}</span>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            {CATEGORIES.map(cat => (
              <button
                ...
                onMouseLeave={() => setCursorVariant('default')}
                className="glass-btn"
                style={{ padding: '10px 20px', borderRadius: '30px', fontSize: '11px', fontWeight: 700, background: selectedCategory === cat.id ? 'var(--accent-color)' : 'var(--glass-bg)' }}
              >
                {cat.name}
              </button>
            ))}
